...
- **2026-08-14:** **Cobrança migrada para a Cakto, e uma migração que ninguém aplicava.** O adaptador de pagamento trocou por trás das mesmas rotas `/api/billing/*`: Cakto é o provedor padrão (`BILLING_PROVIDER=cakto`), Stripe só entra quando alguém escreve `stripe` — **não existe fallback automático**, porque dois provedores ativos cobrariam duas vezes. Consequência operacional: subir o código sem os segredos da Cakto **desliga a compra** em vez de vender pelo Stripe calado. No caminho apareceu o problema maior: `deploy/airia/deploy.sh` aplica exatamente os arquivos listados em `MIGRATION_FILES`, e a migração do PR #10 (`20260811120000_add_objective_intelligence.sql`) **nunca entrou na lista** — mergeada no master, CI verde, e o banco público sem nenhuma das colunas. O próximo deploy teria quebrado Objetivos em produção. Corrigido, com trava genérica: a partir do primeiro arquivo que o deploy aplica, nenhuma migração posterior pode faltar nem sair de ordem. Cadastro financeiro da Cakto, segredos de produção, webhook real e E2E de compra continuam **BLOQUEADOS** — dependem da titular.
