...
import { ArrowLeft, Check, FileText, LoaderCircle, RotateCcw, Sparkles } from 'lucide-react';
    ...
    <main className="routine-builder-page">
      <div className="routine-builder-shell">
        ...
        <section className="routine-panel" aria-busy={busy}>
          ...
          {step === 'source' && !busy && (
            <>
              <h2>{t('routineBuilder.sourceTitle')}</h2>
              ...
              <div className="routine-actions">
                <button className="routine-primary" type="button" disabled={!focus.trim() || (!sourceText.trim() && !file)} onClick={() => void start()}>
                  <Sparkles size={17} /> {t('routineBuilder.readSource')}
                </button>
              </div>
            </>
          )}
          ...
          {step === 'preview' && session?.draftPlan && (
            <>
              ...
              <WeekPreview plan={session.draftPlan} locale={i18n.language === 'en' ? 'en-US' : 'pt-BR'} />
              <div className="routine-actions">
