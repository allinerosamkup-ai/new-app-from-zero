...
export function createApp(dependencies: AppDependencies = {}) {
  ...
  /**
   * POST /api/demo/seed
   * Semeia uma conta autenticada com dados reais de demonstração para uma reunião ou pitch.
   * Não apaga dados da usuária: remove apenas itens previamente marcados como "Airia demo".
   */
  app.post('/api/demo/seed', requireAuth, async (req: Request, res: Response) => {
    const userId = (req as AuthRequest).userId;
    const today = new Date();
    const localToday = new Date(Date.UTC(today.getFullYear(), today.getMonth(), today.getDate(), 12, 0, 0, 0));
    const dateKey = (offset: number) => {
      const d = new Date(localToday);
      d.setUTCDate(localToday.getUTCDate() + offset);
      return d.toISOString().slice(0, 10);
    };
    const dateObj = (offset: number) => parseLocalDateInput(dateKey(offset));

    try {
      await prisma.$transaction(async (tx) => {
        await tx.timelineBlock.deleteMany({
          where: { userId, aiReasoning: { contains: 'Airia demo' } },
        });
        await tx.objective.deleteMany({
          where: { userId, category: 'airia-demo' },
        });
        const demoHabits = await tx.habit.findMany({
          where: { userId, category: 'airia-demo' },
          select: { id: true },
        });
        if (demoHabits.length > 0) {
          await tx.habitCompletion.deleteMany({
            where: { habitId: { in: demoHabits.map((habit) => habit.id) } },
          });
          await tx.habit.deleteMany({
            where: { id: { in: demoHabits.map((habit) => habit.id) } },
          });
        }
        ...
        const checkinPattern = [
          [8, 8, 8, 7, 7, 'Energia clara', 'Finalize a proposta cedo e preserve a tarde.'],
          [7, 7, 7, 6, 8, 'Estável produtiva', 'Bom dia para bloco de foco com limite.'],
          [6, 5, 5, 7, 5, 'Atenção ao atrito', 'Reduza troca de contexto e feche uma pendência.'],
          [4, 3, 3, 8, 4, 'Queda de energia', 'Mover tarefa pesada evita crash no fim do dia.'],
          [3, 3, 2, 8, 4, 'Dia de recuperação', 'Priorize tarefa mínima e descanso programado.'],
          [5, 4, 4, 6, 5, 'Retomada lenta', 'Comece por compromisso reversível.'],
          [7, 6, 6, 4, 7, 'Janela boa', 'Aproveite 50 minutos de execução antes de abrir novas frentes.'],
        ] as const;

        for (let offset = -20; offset <= 0; offset += 1) {
          const pattern = checkinPattern[Math.abs(offset) % checkinPattern.length];
          const [moodScore, energyScore, sleepScore, irritabilityScore, clarityScore, stateLabel, recommendation] = pattern;
          const localDate = dateObj(offset);
          await tx.dailyCheckin.upsert({
            where: {
              userId_localDate_checkinSlot: {
                userId,
                localDate,
                checkinSlot: 'morning-demo',
              },
            },
            update: {
              recordedAt: new Date(`${dateKey(offset)}T08:35:00.000Z`),
              moodScore,
              energyScore,
              sleepScore,
              irritabilityScore,
              clarityScore,
              physicalScore: Math.max(2, Math.min(9, energyScore + 1)),
              socialScore: Math.max(3, Math.min(9, moodScore)),
              factors: moodScore <= 4 ? ['bad_sleep', 'overwhelmed', 'work_pressure'] : ['good_sleep', 'small_win', 'self_trust'],
              emotions: moodScore <= 4 ? ['tired', 'stressed'] : ['focused', 'calm'],
              note: moodScore <= 4
                ? 'Airia demo: acordei com baixa energia e muita pressão, preciso proteger o dia.'
                : 'Airia demo: acordei com mais clareza e quero usar a energia sem exagerar.',
              stateLabel,
              stateLabelType: moodScore <= 4 ? 'sensível' : 'moderado',
              stateSummary: `Airia demo: ${stateLabel}. A leitura cruza humor, energia, sono e carga do planner para orientar o dia.`,
              aiState: {
                stateLabel,
                stateLabelType: moodScore <= 4 ? 'sensível' : 'moderado',
                analysis: `Airia demo: ${stateLabel}. O padrão mostra como sono e carga de tarefas mexem com sua capacidade funcional.`,
                recommendations: [recommendation],
                suggestedIntensity: moodScore <= 4 ? 'L' : energyScore >= 7 ? 'P' : 'M',
                riskSafety: {
                  riskLevel: moodScore <= 3 ? 'moderate' : 'none',
                  signals: moodScore <= 3 ? ['humor e energia muito baixos'] : [],
                  route: moodScore <= 3 ? 'adapt_day' : 'self_support',
                  message: 'Dado demo para demonstrar segurança funcional sem diagnóstico.',
                },
              },
            },
            create: {
        ...
        const timelineItems = [
          ['09:00', '10:10', 'Revisar proposta da Airia', 'trabalho', 'P', 'Tarefa pesada: boa para demonstrar redução ou movimento pela agenda adaptativa.'],
          ['10:30', '11:00', 'Responder mensagem da parceira', 'trabalho', 'M', 'Compromisso real com impacto comercial.'],
          ['12:30', '13:00', 'Almoço sem tela', 'autocuidado', 'L', 'Bloco de proteção de energia.'],
          ['14:00', '14:40', 'Preparar roteiro da ligação', 'pessoal', 'M', 'Bloco ancorado na reunião de segunda.'],
          ['16:00', '16:30', 'Janela de recuperação', 'autocuidado', 'L', 'Airia demo: recuperação planejada, não falha de produtividade.'],
        ] as const;

        for (const [startTime, endTime, title, category, intensity, note] of timelineItems) {
          await tx.timelineBlock.create({
            data: {
              userId,
              localDate: dateObj(0),
              startAt: PlannerService.parseTimeToDate(dateObj(0), startTime),
              endAt: PlannerService.parseTimeToDate(dateObj(0), endTime),
              title,
              category,
              intensity,
              status: 'planned',
              isAiSuggested: title.includes('Janela'),
              aiReasoning: `Airia demo: ${note}`,
              note,
              energyLevel: intensity === 'P' ? 'alta' : intensity === 'L' ? 'leve' : 'media',
            },
          });
        }
        ...
        await tx.objective.create({
          data: {
            userId,
            title: 'Validar Airia com usuárias reais',
            category: 'airia-demo',
            progress: 35,
            description: 'Meta demo: provar que a Airia melhora a semana de quem oscila humor, energia e foco.',
            aiInsight: 'Airia demo: meta ativa usada para gerar sugestões ancoradas, sem inventar tarefa solta.',
            subgoals: [
              { id: 'demo-1', title: 'Preparar demo path de 5 minutos', done: true, aiGenerated: true },
              { id: 'demo-2', title: 'Testar check-in + planner adaptativo', done: false, aiGenerated: true },
              { id: 'demo-3', title: 'Convidar 10 usuárias beta', done: false, aiGenerated: true },
            ],
          },
        });
        ...
        const habits = await Promise.all([
          tx.habit.create({
            data: {
              ...
              description: 'Base diária para a Airia adaptar o dia.',
              category: 'airia-demo',
              icon: '⚡',
              frequency: 'daily',
              timeOfDay: 'morning',
          ...
          tx.habit.create({
            data: {
              userId,
              title: 'Revisão noturna curta',
              description: 'Fecha o ciclo do dia e melhora os insights semanais.',
              category: 'airia-demo',
              icon: '🌙',
              frequency: 'daily',
              timeOfDay: 'evening',
        ...
        for (const habit of habits) {
          for (let offset = -8; offset <= 0; offset += 1) {
            if (habit.title.includes('noturna') && offset % 3 === 0) continue;
            await tx.habitCompletion.create({
              data: {
                habitId: habit.id,
                date: dateObj(offset),
                completedAt: new Date(`${dateKey(offset)}T20:30:00.000Z`),
                moodBefore: 5,
                moodAfter: 6,
              },
            }).catch(() => null);
        ...
        await tx.eventLog.create({
          data: {
            userId,
            eventName: 'demo_mode_loaded',
            properties: {
              checkins: 21,
              timelineBlocks: timelineItems.length,
              habits: habits.length,
              objectives: 1,
              source: 'api_demo_seed',
            },
            path: '/api/demo/seed',
            userAgent: req.get('user-agent') ?? null,
          },
      ...
      return res.status(500).json({ error: 'Failed to seed Airia demo data' });
    }
  });
