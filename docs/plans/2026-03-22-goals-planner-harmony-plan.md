...
## Context
O app tem 13 telas visuais implementadas com Aura v2. Blocos 1-4 do plano anterior já foram executados parcialmente: store expandido com `checkinHistory` + `addCheckin`, check-in flow funcional, Insights/Harmony/DailySummary/Pomodoro conectados ao store, back buttons adicionados. **Porém**, o usuário identificou 7 problemas críticos que precisam ser resolvidos agora:
