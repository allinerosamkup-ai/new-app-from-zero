# Protocolo de Iteração e Conclusão — Airia

> **IMPLEMENTATION IS NOT COMPLETION.
> VERIFICATION IS PART OF IMPLEMENTATION.**

Este documento é obrigatório para qualquer agente que altere código neste
repositório. Ele existe porque o modo de falha mais caro de um agente de código
...
| Esse teste roda na CI? | `apps/backend/package.json` → script `test` é **lista manual** |
...
```
Vitest, 45 arquivos / 316 testes, ~29s. **Descoberta automática** — arquivo novo
`*.test.ts` dentro de `apps/web/src` entra sozinho.

```bash
npm run test -w apps/backend
```
Cadeia manual de `ts-node-transpile-only`, ~90 arquivos, vários minutos.
⚠️ **Arquivo de teste novo NÃO entra sozinho** — precisa ser adicionado à
string do script `test` em `apps/backend/package.json`. Ver
...
Regressão:
- `npm run test -w apps/web` → 316 passed
...
## 21. Enforcement determinístico

Parte deste protocolo é reforçada por hook, não por boa vontade:
...
navegador, preview). No evento `Stop`, se houve alteração de código e nenhuma
verificação, ele bloqueia **uma vez** e devolve a lista dos arquivos alterados.
...
- não bloqueia duas vezes pela mesma pendência, e respeita `stop_hook_active` —
  não existe caminho para laço infinito;
- não bloqueia bloqueio legítimo (§12): rodar qualquer verificação, mesmo que
  falhe, libera a parada — falha verificada é informação, e informação é o que o
  protocolo quer.
...
Hooks carregam no início da sessão: alterar o hook exige reiniciar o Claude Code.
